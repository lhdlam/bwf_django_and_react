...
from .serielizers import GroupSerializer, EventSerializer, GroupFullSerializer, UserSerializer, \
    UserProfileSerializer, ChangePasswordSerializer, MemberSerializer, \
    CommentSerializer, EventFullSerializer, BetSerializer
...
class BetViewset(viewsets.ModelViewSet):
    ...
    def create(selfself, request, *arg, **kwargs):
        response = {"message": "Method not allowed"}
        return Response(response, status=status.HTTP_405_METHOD_NOT_ALLOWED)

    def update(selfself, request, *arg, **kwargs):
        response = {"message": "Method not allowed"}
        return Response(response, status=status.HTTP_405_METHOD_NOT_ALLOWED)
        ...
        if 'event' in request.data and 'score1' in request.data and 'score2' in request.data:
            ...
            # if in_group:
                ...
                try:
                    ...
                    response = {"message": "Bet Update", "new": False, "result": serializer.data}
                    return Response(response, status=status.HTTP_200_OK)
                except:
                    my_bet = Bet.objects.create(event=event, user=request.user, score1=score1, score2=score2)
                    serializer = BetSerializer(my_bet, many=False)
                    response = {"message": "Bet Created", "new": False, "result": serializer.data}
                    return Response(response, status=status.HTTP_200_OK)
